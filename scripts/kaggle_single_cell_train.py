...
# Kaggle's default pre-installed PyTorch (confirmed via 5 real failed runs on
# 2026-09-07: torch 2.10.0+cu128) has narrowed its prebuilt wheel's compiled
# kernel images to compute capability 7.0+ -- it does NOT include sm_60
# (Pascal), which is what a Tesla P100 needs. Every one of those 5 runs was
# assigned a P100 and crashed identically on the very first forward pass with
# "CUDA error: no kernel image is available for execution on the device".
# This is not a random GPU-assignment fluke to retry past -- it's a
# persistent mismatch between Kaggle's current default torch build and
# whatever older-generation GPU (P100) the kernel pool happens to hand out.
# torch==2.4.1 is the most recent release still shipping sm_60 support in its
# official wheels, so pin to that instead of trusting whatever's preinstalled.
# --extra-index-url (not --index-url) so pip can still resolve torch's own
# build dependencies (filelock, sympy, jinja2, etc.) from PyPI rather than
# only from download.pytorch.org, which doesn't host every dependency.
subprocess.run(

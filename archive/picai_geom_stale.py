...
def _find_label_path(labels_dir: Path, case_id: str, kind: str) -> Path:
    """kind: 'gland', 'zone', or 'lesion'. Tries .nii.gz first (how
    05_package_for_kaggle.py ships them locally), then bare .nii --
    Kaggle's dataset ingestion was found to silently decompress gzip'd
    files uploaded inside a zip-mode directory (confirmed via
    `kaggle datasets files` after uploading: labels/.../*_gland.nii.gz
    came back as labels/.../*_gland.nii, ~3x larger, uncompressed).
    Handling both means this code works whether or not that happens."""
